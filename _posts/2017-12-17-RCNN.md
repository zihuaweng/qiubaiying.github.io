...
\\( d_x(P),d_y(P),d_w(P),d_h(P) \\)是\\( \O_5(P) \\)的线性方程。\\( w_* \\)通过下面学习得到：
\\[ w_*=\underset{w}{argmin}\sum_{i}^{N}(t^i_*-\widehat{w}^T_*\O_5(P^i))^2+\lambda\left\|\widehat{w}_*\right\|^2 \\]
其中：
1. \\( t_* \\)为预测P与GT的偏差，由下面公式计算得到：
